...
<form method="POST" action="{{ route('merchant.products.store') }}">
    ...
    <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
        <div class="border-b border-slate-200 px-6 py-4">
            <h2 class="text-base font-semibold text-slate-800">Product Information</h2>
        </div>
        <div class="grid grid-cols-1 gap-6 p-6 sm:grid-cols-2">

            <div class="sm:col-span-2">
                <label for="name" class="{{ $label }}">Name <span class="text-red-500">*</span></label>
                <input id="name" name="name" type="text" value="{{ old('name') }}" class="{{ $input }}" placeholder="e.g. Classic Burger">
            ...
            <div class="sm:col-span-2">
                ...
                <input id="image_url" name="image_url" type="url" value="{{ old('image_url') }}" class="{{ $input }}" placeholder="https://…">
