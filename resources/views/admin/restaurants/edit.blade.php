...
<div class="mb-6 flex items-center gap-2 text-sm text-slate-500">
    ...
    <span>/</span>
    <span class="truncate font-medium text-slate-800">{{ $restaurant->name }}</span>
</div>
...
<form method="POST" action="{{ route('admin.restaurants.update', $restaurant) }}">
    ...
    <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
        ...
        <div class="border-b border-slate-200 px-6 py-4">
            <h2 class="text-base font-semibold text-slate-800">Edit Restaurant</h2>
            <p class="mt-0.5 text-sm text-slate-500">Update brand-level information for <span class="font-medium text-slate-700">{{ $restaurant->name }}</span>.</p>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 p-6 sm:grid-cols-2">
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-slate-400">2-letter ISO 3166-1 alpha-2 code.</p>
        ...
        <div class="flex items-center justify-between gap-3 border-t border-slate-200 bg-slate-50/60 px-6 py-4">
            <button type="button"
                    onclick="if(confirm('Permanently delete {{ addslashes($restaurant->name) }}?')) document.getElementById('delete-form').submit()"
